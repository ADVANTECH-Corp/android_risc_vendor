 ...
 *
 * $Id: dhd_linux.c 608724 2015-12-29 06:12:28Z $
 */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25)) && defined(OEM_ANDROID) && 1
...
#endif /* CUSTOMER_HW4 && ARGOS_CPU_SCHEDULER && ARGOS_DPC_TASKLET_CTL */
		/*  use tasklet for dpc */
...
#else
	sprintf(memdump_path, "%s_%s_%ld.%ld", "/tmp/mem_dump",
		memdump_type, (unsigned long)curtime.tv_sec,
		(unsigned long)curtime.tv_usec);
...
#if defined(DHD_L2_FILTER) || defined(BCM_ROUTER_DHD)
...
#endif /* BCM_ROUTER_DHD && BCM_GMAC3 && DHD_WMF */
