 ...
 *
 * $Id: wlu.c 609356 2016-01-04 03:49:38Z $
 */
...
static cmd_func_t wl_rand, wl_counters, wl_if_counters, wl_clear_counters;
...
static cmd_func_t wl_varstr, wl_iov_del_route, wl_iov_add_route,
wl_iov_mesh_filter, wl_iov_mesh_peer, wl_iov_mesh_peer_status, wl_iov_max_peers;
static cmd_func_t wl_iov_sae_password;
static cmd_func_t wl_iov_mesh_delfilter, wl_iov_mesh_find;
...
static int wl_nsc_subtypes(void *wl, cmd_t *cmd, char **argv);
...
cmd_t wl_cmds[] = {
	...
	{"frag_dur", wl_frag_dur, WLC_GET_VAR, WLC_SET_VAR,
	"Adjusts frame fragmentation threshold to limit maximum transmission \n"
	"\tduration.\n"
	"\tUsage: wl frag_dur [frag_dur_us].\n"
	"\t. where [frag_dur_us] is an unsigned 16-bit integer representing \n"
	"\t  maximum transmission duration in microseconds.\n"
	"\t. default value: [frag_dur_us] = 0.\n"
	"\t. with [frag_dur_us] = 0, frame fragmentation threshold calculations \n"
	"\t  bypass this iovar."},
	...
	"\tUsage: join <ssid> [key <0-3>:xxxxx] [imode bss|ibss|mesh] "
	...
	{ "mesh_add_route", wl_iov_add_route, WLC_GET_VAR, WLC_SET_VAR,
	"Set Mesh route\n"
	"\tUsage: wl mesh_add_route <dest-address> <nexthop address>\n"
	},
	{ "mesh_del_route", wl_iov_del_route, WLC_GET_VAR, WLC_SET_VAR,
	"Delete Mesh route\n"
	"\tUsage: wl mesh_del_route <dest-address> \n"
	},
	{ "mesh_find", wl_iov_mesh_find, WLC_GET_VAR, WLC_SET_VAR,
	"Filter Address\n"
	"\tUsage: wl mesh_find <dest-address> \n"
	},
	{ "mesh_filter", wl_iov_mesh_filter, WLC_GET_VAR, WLC_SET_VAR,
	"Filter Address\n"
	"\tUsage: wl mesh_filter <dest-address> \n"
	},
	{ "mesh_peer", wl_iov_mesh_peer, -1, WLC_SET_VAR,
	"peer or cancel peering with a given mac address OR list present addresses in the list\n"
	"\tUsage: [1] to add: wl mesh_peer <dest-address> ... \n"
	"\t\t[2] to get present list: wl mesh_peer"
	"\t\t[3] to cancel all the peerings: wl mesh_peer cancel"
	"\t\t[4] to cancel peering with a particular peer: wl mesh_peer <dest-address> ... \n"
	},
	{ "mesh_peer_status", wl_iov_mesh_peer_status, WLC_GET_VAR, WLC_SET_VAR,
	"list of peered addresses along with all data\n"
	"\tUsage: wl mesh_peer_status \n"
	},
	{ "mesh_delfilter", wl_iov_mesh_delfilter, WLC_GET_VAR, WLC_SET_VAR,
	"Delete Filter Address\n"
	"\tUsage: wl mesh_delfilter <dest-address> \n"
	},
	{ "mesh_max_peers", wl_iov_max_peers, WLC_GET_VAR, WLC_SET_VAR,
	"Sets the maximum number of peerings possible\n"
	"\tUsage:wl mesh_max_peers <max no. of peerings>"
	"Gives the current value of mesh_max_peers(default value = 8)"
	"\tUsage:wl mesh_max_peers"
	},
	{ "sae_password", wl_iov_sae_password, WLC_GET_VAR, WLC_SET_VAR,
	"Set Sae password\n"
	"\tUsage: wl sae_password <password string>\n"
	},
	...
	"\tusage: wl pmac <type> <addresses up to 16> -s <step size> -n <num> -b <bitmap> -r\n"
	...
	"\tTo SET data supply datastring\n"
	"\t\t Example: wl pfn_nsc_afdata \"datastring\" \n"
	"\t\t Note: data contents are user specific\n"
	},
	{ "pfn_nsc_rstdef", wl_nsc_rstdef, -1, WLC_SET_VAR,
	"Restores default config for nsc.\n"
	},
	{ "pfn_nsc_afdstmac", wl_iov_mac, -1, -1,
	"Get/set the current action frame dst address"
	},
	{ "pfn_nsc_afbssid", wl_iov_mac, -1, -1,
	"Get/set the current action frame bssid"
	},
	{ "pfn_nsc_subtypes", wl_nsc_subtypes, -1, -1,
	"Get/set the action frame allowed subtype values\n"
	"\tWith no args returns the list of subtypes\n"
	"\tTo set array values:\n"
	"\t supply space separated integer args, up to 8 total\n"
	},
#endif /* CUSTOMER_HW_31_1 */
	...
	"\n\t Sub command \"cnt\": wl peer_rssi cnt [nnn]\n"
	"\t\t to enable or disable peer rssi statistics feature\n"
	"\t\t none-zero nnn to set the max number of peers that can be monitored;\n"
	"\t\t zero nnn to free the data base and disable the feature;\n"
	"\t\t omit nnn to query current peers count and max number of peers wat set;\n"
	"\n\t Sub command \"reg\": wl peer_rssi reg <MAC_ADDR> [nnn]\n"
	"\t\t to register or unregister a peer to or from the rssi statistics data base\n"
	"\t\t none-zero nnn to set the max rssi sample count for the peer\n"
	"\t\t zero nnn to remove the peer from the monitor list\n"
	"\t\t omit nnn to query the max rssi sample count that was set for the peer;\n"
	"\n\t Sub command \"rssi\": wl peer_rssi rssi <MAC_ADDR>\n"
	"\t\t to query the average rssi over the collected number of samples for the peer\n"
...
static int
wl_iov_max_peers(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	uint8 max_peer = 0;
	if (argv[1]) {
		max_peer = (uint8)atoi(argv[1]);
		printf("MAX_PEER_WLU=%d", max_peer);
		return wlu_iovar_set(wl, cmd->name, &max_peer, sizeof(uint8));
	}
	else {
		ret = wlu_iovar_get(wl, cmd->name, &max_peer, sizeof(uint8));
		printf("CURRENT MAXIMUM PEERS = %d\n", max_peer);
		return ret;
	}
}

static int
wl_iov_add_route(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	struct ether_addr ea[2];

	if (argv[2]) { /* set */
		if (!wl_ether_atoe(argv[1], &ea[0])) {
			printf(" ERROR: no valid ether addr provided\n");
			return -1;
		}

		if (!wl_ether_atoe(argv[2], &ea[1])) {
			printf(" ERROR: no valid ether addr provided\n");
			return -1;
		}

		if ((ret = wlu_iovar_set(wl, cmd->name, &ea, ETHER_ADDR_LEN*2)) < 0) {
			printf("Error setting variable %s\n", argv[0]);
			return ret;
		}
		return 0;
...
static int
wl_iov_sae_password(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	char password[SAE_MAX_PASSWD_LEN];

	if (argv[1]) { /* set */
		if (strlen(password) >= SAE_MAX_PASSWD_LEN) {
			printf("Password length is high\n");
			return -1;
		}

		strcpy(password, argv[1]);

		if ((ret = wlu_iovar_set(wl, cmd->name, password, strlen(password) + 1)) < 0) {
			printf("Error setting variable %s\n", argv[0]);
			return ret;
		}
		return 0;
...
static int
wl_iov_del_route(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	struct ether_addr ea = {{0, 0, 0, 0, 0, 0}};
...
static int
wl_iov_mesh_filter(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	struct ether_addr ea = {{0, 0, 0, 0, 0, 0}};
...
static int
wl_iov_mesh_delfilter(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	struct ether_addr ea = {{0, 0, 0, 0, 0, 0}};
	if (argv[1]) { /* set */
...
static int wl_iov_mesh_find(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	struct ether_addr ea = {{0, 0, 0, 0, 0, 0}};
...
wl_mesh_print_peer_info(mesh_peer_info_ext_t *mpi_ext, uint32 peer_results_count)
{
	...
	printf("no: ------addr------ : l.aid : state : p.aid :"
	...
	for (count = 0; count < peer_results_count; count++) {
		if (mpi_ext->entry_state != MESH_SELF_PEER_ENTRY_STATE_TIMEDOUT)
			printf("%d: %s : 0x%x : %s : 0x%x : %4d : %4d : %4d : %s : %d\n",
				count, wl_ether_etoa(&mpi_ext->ea), mpi_ext->local_aid,
...
static void
wl_print_maclist(struct peer_maclist *maclist)
{
	struct ether_addr *ea;
	uint i = 0;

	ea = &maclist->ea[0];
	printf("maclist count: %d\n", maclist->count);
...
/* note: used for both peering and cancel-peering */
static int
wl_iov_mesh_peer(void *wl, cmd_t *cmd, char **argv)
{
	struct ether_addr *ea;
	struct peer_maclist *maclist = (struct peer_maclist *) buf;
	uint max = (WLC_IOCTL_MAXLEN - sizeof(int)) / ETHER_ADDR_LEN;
	uint len = 0;
	uint ret = 0;
	maclist->count = htod16(0);
	maclist->flag  = htod16(0);
	if (argv[1]) { /* set */
		ea = &maclist->ea[0];
		if (!stricmp(*++argv, "cancel")) {
					/* mesh_peer cancel can be done selectively for */
					/* multiple mac addresses after cancel string */
					/* and will be taken care in common code part below  */
					maclist->flag = PEER_MACLIST_FLAG_CLEAR;
					if (!(*++argv)) {
						return wlu_iovar_set(wl, cmd->name,
							maclist, sizeof(maclist));
					}
			}
			...
			wl_print_maclist(maclist);
			/* Set new list */
			len = sizeof(maclist->flag) + sizeof(maclist->count)
				+ maclist->count * sizeof(maclist->ea);
			maclist->count = htod16(maclist->count);
			return wlu_iovar_set(wl, cmd->name, maclist, len);
	} else {
			mesh_peer_info_dump_t *peer_results = (mesh_peer_info_dump_t *) buf;
			mesh_peer_info_ext_t *mpi_ext;
			/* get table and print */
			ret = wlu_iovar_get(wl, cmd->name, peer_results, WLC_IOCTL_MAXLEN);
			if (!ret) {
				mpi_ext = peer_results->mpi_ext;
				wl_mesh_print_peer_info(mpi_ext, peer_results->count);
			}
		}
		return ret;
	}
...
static int
wl_iov_mesh_peer_status(void *wl, cmd_t *cmd, char **argv)
{
	char *dump_buf;
	int bcmerr = 0;
	int indata;
	...
	indata = htod32(WL_DUMP_BUF_LEN);
	inlen = 4;
	/* query for the 'dump' without any argument */
	bcmerr = wlu_iovar_getbuf(wl, "mesh_peer_status", &indata, inlen,
	...
	if (ret != BCME_OK) {
		...
		if (ret != BCME_OK) {
			...
			do {
				...
				if ((ret = wlu_iovar_getbuf(wl, "dump", dump_buf, strlen(dump_buf),
...
wl_dfs_ap_move(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (abort == -1) {
		err = wlu_iovar_setint(wl, cmd->name, abort);
	} else if (chanspec != 0) {
		val = wl_chspec32_to_driver(chanspec);
		if (val != INVCHANSPEC) {
			err = wlu_iovar_setint(wl, cmd->name, val);
...
wl_nsc_afdata(void *wl, cmd_t *cmd, char **argv)
{
	...
	datalen = strlen(argv[1])/2;	/* ascii --> hex */

	if ((err = get_ie_data ((uchar *)argv[1], (void *)pafb, datalen))) {
		fprintf(stderr, "Error parsing data arg\n");
		return err;
	}

	len = datalen + sizeof(nsc_af_body_t) - 1 + cmdlen;
...
static int
wl_nsc_subtypes(void *wl, cmd_t *cmd, char **argv)
{
	uint8 sp_arr[NSC_SUBTYPES_MAX];
	int retval = 0;
	int i;
	...
	if (!(argv[1])) {
		/* Get: */
		retval = wlu_iovar_getbuf(wl, "pfn_nsc_subtypes", sp_arr,
			sizeof(sp_arr), buf, WLC_IOCTL_SMLEN);
		if (retval) {
			printf("%s:%d: ERROR getting afsubtypes array\n", __FUNCTION__, __LINE__);
			return retval;
		}
		printf("AF Subtypes array:\n");
		for (i = 0; i < NSC_SUBTYPES_MAX; i++) {
			printf("subtype[%d] = %d\n", i, ((uint8 *)buf)[i]);
		}
		return 0;
	}
	...
	for (i = 0; i < NSC_SUBTYPES_MAX; i++) {
		if (!argv[i + 1]) {
			sp_arr[i] = NSC_SUBTYPES_EOL;
			break;
		}
		sp_arr[i] = strtoul(argv[i + 1], NULL, 0);
	}

	retval = wlu_iovar_setbuf(wl, "pfn_nsc_subtypes", sp_arr,
			sizeof(sp_arr), buf, WLC_IOCTL_SMLEN);

	if (retval) {
		printf("ERROR: %d setting subtypes array\n", retval);
	}
	return retval;
}
static int
...
static int
wl_peerrssi(void *wl, cmd_t *cmd, char **argv)
{
	wl_peer_rssi_t peer_rssi;
	int buflen = 0;
	char *endptr = NULL;
	int err;
	int set = 0;
	...
	buflen = strlen(*argv);
	memset(buf, 0, buflen + 1 + sizeof(peer_rssi));
	memcpy(buf, *argv, buflen);
	buflen += 1;

	/* toss the command name "peer_rssi" */
	argv++;

	if (*argv == NULL) {
		printf("ERROR: peer_rssi sub command not provided\n");
		return BCME_USAGE_ERROR;
	}

	peer_rssi.version = WLC_IOCTL_VERSION_PEER_RSSI;
	if (!strcmp(*argv, "cnt")) {
		cmd_cnt = 1;
		peer_rssi.cmd = WLC_IOCTL_CMD_PEER_RSSI_PEERS_CNT;
	} else if (!strcmp(*argv, "reg")) {
		cmd_reg = 1;
		peer_rssi.cmd = WLC_IOCTL_CMD_PEER_RSSI_PEER_REG;
	} else if (!strcmp(*argv, "rssi")) {
		cmd_rssi = 1;
		peer_rssi.cmd = WLC_IOCTL_CMD_PEER_RSSI_PEER_RSSI;
	} else {
		printf("ERROR: peer_rssi cmd \"%s\" not recognized\n", *argv);
		return BCME_USAGE_ERROR;
	}

	argv++; /* toss the peer_rssi sub command */

	if (cmd_reg || cmd_rssi) {
		if (*argv == NULL) {
			printf("ERROR: MAC address not provided\n");
			return BCME_BADARG;
		}
		if (!wl_ether_atoe(argv[0], &peer_rssi.ea)) {
			printf("ERROR: MAC address not valid\n");
			return BCME_BADARG;
		}
		argv++; /* toss peer MAC address */
	}

	if (*argv) {
		set = 1;
		peer_rssi.max_cnt = strtol(*argv, &endptr, 0);

		if (*endptr != '\0') {
			return BCME_BADARG;
		}

		if (cmd_cnt && ((peer_rssi.max_cnt < 0) ||
			(peer_rssi.max_cnt > (MAXNBVAL(sizeof(uint8)) - 1)))) {
			printf("ERROR: max peer count (%d) wrong,"
				" valid through [0, ..., 254]\n",
				peer_rssi.max_cnt);
			return BCME_BADARG;
		}
		if (cmd_reg && ((peer_rssi.max_cnt < 0) ||
			(peer_rssi.max_cnt > (MAXNBVAL(sizeof(uint8)) - 1)))) {
			printf("ERROR: max sample count (%d) wrong,"
				" valid through [0, ..., 254]\n",
				peer_rssi.max_cnt);
			return BCME_BADARG;
		}
		if (cmd_rssi) {
			printf("ERROR: not supported, cann't set peer statistics rssi\n");
			return BCME_BADARG;
		}
	}

	peer_rssi.version = htod32(peer_rssi.version);
	peer_rssi.cmd = htod32(peer_rssi.cmd);
	peer_rssi.max_cnt = htod32(peer_rssi.max_cnt);
	peer_rssi.cur_val = htod32(peer_rssi.cur_val);

	memcpy(buf + buflen, &peer_rssi, sizeof(peer_rssi));
	buflen += sizeof(peer_rssi);

	if (set) {
		err = wlu_set(wl, cmd->set, &buf[0], buflen);
	} else {
		err = wlu_get(wl, cmd->get, &buf[0], buflen);
		if (err)
			return err;
		memcpy(&peer_rssi, buf, sizeof(peer_rssi));

		if (cmd_cnt) {
			printf("Now doing rssi statistics for %d (max %d) peers\n",
				dtoh32(peer_rssi.cur_val), dtoh32(peer_rssi.max_cnt));
		} else if (cmd_reg) {
			printf("Max count of rssi samples for the peer is %d\n",
				dtoh32(peer_rssi.max_cnt));
		} else if (cmd_rssi) {
			printf("Average rssi value for the peer is %d over %d samples\n",
				dtoh32(peer_rssi.cur_val), dtoh32(peer_rssi.max_cnt));
		}
	}
	...
	if (!*++argv) {
		...
		if (core_idx) {
			for (i = 0; i < WL_TXPWR_CORE_MAX; i++) {
				switch (unit) {
				case UNIT_MW:
					pwr[i] = (uint8)wl_mw_to_qdbm((uint16)MIN(pwr[i], 0xffff));
					break;
				case UNIT_DBM:
					if (pwr[i] > (WLC_TXPWR_MAX / WLC_TXPWR_DB_FACTOR))
						return BCME_BADARG;
					pwr[i] *= WLC_TXPWR_DB_FACTOR;
					if (pwr[i] < WL_RATE_DISABLED)
						pwr[i] = WL_RATE_DISABLED;
					temp_val = (int8)pwr[i];
					/* need to keep sign bit in low byte */
					pwr[i] = (uint8)temp_val;
					break;
				case UNIT_QDBM:
...
static int
wl_frag_dur(void *wl, cmd_t *cmd, char **argv)
{
	uint32 frag_dur;
	int ret;
	void *ptr = NULL;
	...
	if (*++argv) {
		/* set */
		frag_dur = htod32(atoi(*argv));
		return (wlu_var_setbuf(wl, cmd->name, &frag_dur, sizeof(uint32)));
	} else {
		/* get */
		if ((ret = wlu_var_getbuf(wl, cmd->name, NULL, 0, &ptr)) == 0)
			printf("frag_dur = %u microseconds.\n", dtoh32(*(uint32 *)ptr));
		return (ret);
	}
}
...
wl_sta_info(void *wl, cmd_t *cmd, char **argv)
{
	...
	printf("\t flags 0x%x:%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n",
	       sta->flags,
...
static int wl_counters_cbfn(void *ctx, uint8 *data, uint16 type, uint16 len)
{
	...
	switch (type) {
		case WL_CNT_XTLV_WLC: {
			...
			PRVALSF(tbtt); PRVALSF(txdatamcast); PRVALSF(txdatabcast); PRNL();
...
#ifdef BT_WIFI_HANDOVER
static void
wl_counters_bwte_print(bwte_stats_t *stats)
{
	char *pbuf = buf;

	pbuf += sprintf(pbuf, "%s %u ", "inttobt", dtoh32(stats->inttobt));
	pbuf += sprintf(pbuf, "%s %u ", "intfrombt", dtoh32(stats->intfrombt));
	pbuf += sprintf(pbuf, "\n");

	pbuf += sprintf(pbuf, "%s %u ", "wl2btdset", dtoh32(stats->wl2bt_dset_cnt));
	pbuf += sprintf(pbuf, "%s %u ", "wl2btdclear", dtoh32(stats->wl2bt_dclear_cnt));
	pbuf += sprintf(pbuf, "%s %u ", "wl2btaset", dtoh32(stats->wl2bt_aset_cnt));
	pbuf += sprintf(pbuf, "%s %u ", "wl2btaclear", dtoh32(stats->wl2bt_aclear_cnt));
	pbuf += sprintf(pbuf, "\n");

	pbuf += sprintf(pbuf, "%s %u ", "bt2wldset", dtoh32(stats->bt2wl_dset_cnt));
	pbuf += sprintf(pbuf, "%s %u ", "bt2wldclear", dtoh32(stats->bt2wl_dclear_cnt));
	pbuf += sprintf(pbuf, "%s %u ", "bt2wlaset", dtoh32(stats->bt2wl_aset_cnt));
	pbuf += sprintf(pbuf, "%s %u ", "bt2wlaclear", dtoh32(stats->bt2wl_aclear_cnt));
	pbuf += sprintf(pbuf, "\n");

	pbuf += sprintf(pbuf, "%s %u ", "error1", dtoh32(stats->state_error_1));
	pbuf += sprintf(pbuf, "%s %u ", "error2", dtoh32(stats->state_error_2));
	pbuf += sprintf(pbuf, "%s %u ", "error3", dtoh32(stats->state_error_3));
	pbuf += sprintf(pbuf, "%s %u ", "error4", dtoh32(stats->state_error_4));
	pbuf += sprintf(pbuf, "\n");
...
#ifdef BT_WIFI_HANDOVER
	bwte_stats.version = htod16(WL_BWTE_STATS_VERSION);
	if (wlu_var_getbuf(wl, "bwte_stats", &bwte_stats, sizeof(bwte_stats_t),
			&ptr) == BCME_OK) {
		memcpy(&bwte_stats, ptr, sizeof(bwte_stats_t));
		wl_counters_bwte_print(&bwte_stats);
	}
#endif /* BT_WIFI_HANDOVER */
...
wl_clear_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* make up nan cmd ioctl header */
	nanioc->version = htod16(WL_NAN_IOCTL_VERSION);
	nanioc->id = WL_NAN_CMD_CLEARCOUNT;
	nanioc->len = NAN_IOC_BUFSZ;
...
#ifdef BT_WIFI_HANDOVER
	bwte_stats.version = htod16(WL_BWTE_STATS_VERSION);
	wlu_var_getbuf(wl, "bwte_stats_clear", &bwte_stats, sizeof(bwte_stats_t), &ptr);
#endif /* BT_WIFI_HANDOVER */
...
wl_wowl_wakeind(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (wake.ucode_wakeind != 0) {
		...
		if ((wake.ucode_wakeind & WL_WOWL_MIC_FAIL) == WL_WOWL_MIC_FAIL)
			printf("\tWake for MIC countermeasures.\n");
