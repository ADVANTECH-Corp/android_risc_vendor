...
wl_wnm_bss_select_table(void *wl, cmd_t *cmd, char **argv)
{
	...
	btcfg = malloc(sizeof(wnm_bss_select_factor_cfg_t) +
	...
	if ((argc == 2) && (btcfg->band != WLC_BAND_AUTO)) {
		...
		if ((ret = wlu_var_getbuf(wl, cmd->name, btcfg, sizeof(btcfg), &ptr)) < 0) {
			free(btcfg);
			return ret;
		}

		memcpy(btcfg, ptr, sizeof(*btcfg));
		memcpy(btcfg, ptr, btcfg->count * sizeof(*btcfg));
		...
		ret = wlu_var_setbuf(wl, cmd->name, btcfg, sizeof(*btcfg) +
			(btcfg->count - 1) * sizeof(wnm_bss_select_factor_params_t));
	}
