...
#ifdef SJL_FIXME
DECL_CMDHANDLER(collect);			/* session-only */
DECL_CMDHANDLER(tune);				/* method-only; */
#endif /* SJL_FIXME */
...
#ifdef SJL_FIXME
	/* (set) wl proxd ftm <session-id> collect */	/* SJL_FIXME */
	{"collect", WL_PROXD_CMD_COLLECT, FTM_SUBCMD_FUNC(collect),
	FTM_SUBCMD_FLAG_SESSION, "Collect debug info of FTM measurement (TBD)" },
	/* wl proxd ftm tune */							/* SJL_FIXME */
	{"tune", WL_PROXD_CMD_TUNE, FTM_SUBCMD_FUNC(tune),
	FTM_SUBCMD_FLAG_METHOD, "Tune " }
#endif /* SJL_FIXME */
};
...
*/
static void
ftm_unpack_and_display_rtt_result(uint8 *p_data, uint16 tlvid, uint16 len)
{
	int		i;
	char	dispbuf[256];
	char	*p_dest;
...
*/
static void
ftm_unpack_and_display_session_info(uint8 *p_data, uint16 tlvid, uint16 len)
{
...
*/
static void
ftm_unpack_and_display_session_status(uint8 *p_data, uint16 tlvid, uint16 len)
{
...
*/
static void
ftm_unpack_and_display_counters(uint8 *p_data, uint16 tlvid, uint16 len)
{
...
*/
static void
ftm_unpack_and_display_session_idlist(uint8 *p_data, uint16 tlvid, uint16 len)
{
...
*/
static void
ftm_unpack_and_display_avail(uint8 *p_data, uint16 tlvid, uint16 len)
{
...
*/
static void
ftm_unpack_and_display_ranging_info(uint8 *p_data, uint16 tlvid, uint16 len)
{
 ...
 */
static int
ftm_unpack_xtlv_cbfn(void *ctx, uint8 *p_data, uint16 tlvid, uint16 len)
{
	int res = BCME_OK;
	char dispbuf[256];
...
#ifdef SJL_FIXME
/*
* request a 'collect' for a session (SJL_FIXME)
* Usage: wl proxd ftm <session-id> collect
*
* Note, caller should have verify the session-id before this call.
*/
static int
ftm_subcmd_collect(void *wl, const ftm_subcmd_info_t *p_subcmd_info,
	wl_proxd_method_t method, wl_proxd_session_id_t session_id, char **argv)
{
	/* issue an iovar call */
	return ftm_subcmd_setiov_no_tlv(wl, p_subcmd_info, method, session_id, argv);
...
/*
* Usage: wl proxd ftm tune (SJL_FIXME)
*/
static int ftm_subcmd_tune(void *wl, const ftm_subcmd_info_t *p_subcmd_info,
	wl_proxd_method_t method, wl_proxd_session_id_t session_id, char **argv)
{
	/* issue an iovar call */
	return ftm_subcmd_setiov_no_tlv(wl, p_subcmd_info, method, session_id, argv);
...
static const ftm_config_options_info_t ftm_session_options_info[] = {
	...
	{ "tof-compat",	WL_PROXD_SESSION_FLAG_TOF_COMPAT,
	"TOF compatibility (not currently supported)" },
#ifdef SJL_FIXME
...
#ifdef SJL_FIXME
	{ "ts1", WL_PROXD_SESSION_FLAG_TS1, " e.g. FTM1 - cap or rx" }, /* readonly */
	...
	{ "no-chansw", WL_PROXD_SESSION_FLAG_NOCHANSWT, "no channel switching for RTT" },
	...
	{ "collect",	WL_PROXD_SESSION_FLAG_COLLECT, "debug -- collect" },
	...
	wl_proxd_tlv_t **p_tlv, uint16 *p_buf_space_left)
{
	...
	/* if <session-id> is specified, use session-flags, otherwise use 'global/method' */
	useMethod = (session_id == WL_PROXD_SESSION_ID_GLOBAL) ? TRUE : FALSE;
	...
	status = bcm_pack_xtlv_entry((uint8 **)p_tlv, p_buf_space_left,
		...
		sizeof(uint32), &flags_mask, BCM_XTLV_OPTION_ALIGN32);
	if (status != BCME_OK) {
#ifdef WL_FTM_DEBUG
	...
	status = bcm_pack_xtlv_entry((uint8 **)p_tlv, p_buf_space_left,
		...
		sizeof(uint32), &flags, BCM_XTLV_OPTION_ALIGN32);
	if (status != BCME_OK) {
#ifdef WL_FTM_DEBUG
	...
	wl_proxd_tlv_t **p_tlv, uint16 *p_buf_space_left)
{
	...
	err = bcm_pack_xtlv_entry((uint8 **) p_tlv, p_buf_space_left,
		...
		ranging_sids, BCM_XTLV_OPTION_ALIGN32);
...
wl_proxd_tlv_t **p_tlv, uint16 *p_buf_space_left)
{
	...
	if (flags != WL_PROXD_RANGING_FLAG_NONE) {
		...
		err = bcm_pack_xtlv_entry((uint8 **) p_tlv, p_buf_space_left,
			...
			sizeof(uint16), &flags, BCM_XTLV_OPTION_ALIGN32);
		if (err != BCME_OK) {
		...
		err = bcm_pack_xtlv_entry((uint8 **) p_tlv, p_buf_space_left,
			...
			sizeof(uint16), &flags_mask, BCM_XTLV_OPTION_ALIGN32);
		if (err != BCME_OK) {
