...
struct config_iovar_s config_iovar_list[] = {
	{ "rsdb_mode", {{"auto", -1}, {"mimo", 0}, {"rsdb", 1}, {"80p80", 2},
	{NULL, 0}}},
...
int wlmSelectIntfidx(char *opt, char *idx)
{
	...
	} else if (!stricmp(opt, "-W") || !strcmp(opt, "--wlc")) {
		char *endptr = NULL;
		g_wlc_idx = strtol(idx, &endptr, 0);
	} else {
		g_wlc_idx = -1;
		strncpy(g_rem_ifname, "", IFNAMSIZ);
	}
	...
	unsigned int *b0, unsigned int *b1, unsigned int *b2, unsigned int *b3)
{
	...
	if (wlu_iovar_getint(irh, "paparambwver", &paparambwver) < 0) {
		printf("wlmACPaParametersSRRev12Get: Cannot find any paparam bandwith version\n");
		return FALSE;
	}
	...
	if (sromrev != 12) {
		printf("wlmACPaParametersSRRev12Get: srom revision is not 12\n");
		return FALSE;
	}
...
int wlmTxGetAckedPackets(unsigned int *count)
{
	...
	if (wlu_var_getbuf(irh, "counters", NULL, 0, &cntbuf)) {
		printf("wlmTxGetAckedPackets: %s\n", wlmLastError());
		return FALSE;
	}

	/* Translate traditional (ver <= 10) counters struct to new xtlv type struct */
	res = wl_cntbuf_to_xtlv_format(NULL, cntbuf, WL_CNTBUF_MAX_SIZE, 0);
	if (res != BCME_OK) {
	...
	if ((macstat_cnt = MCSTCNT_LE10_FROM_CNTBUF(cntbuf)) == NULL) {
		macstat_cnt = MCSTCNT_GT10_FROM_CNTBUF(cntbuf);
	}
...
int wlmRxGetReceivedPackets(unsigned int *count)
{
	...
	if (cntinfo->version > WL_CNT_VERSION_6) {
		*count = dtoh32(macstat_cnt->pktengrxducast);
	} else {
		*count = dtoh32(macstat_cnt->rxdfrmucastmbss);
	}
...
int wlmScanNetworks(char* results_buf, int len)
{
	...
	if (params == NULL) {
		printf("wlmScanNetworks: Error alloating %d bytes for scan params\n", params_size);
		return FALSE;
	}
	...
	if (dump_buf == NULL) {
		printf("wlmScanNetworks: Error allocat %d bytes for dump_buf\n", WLM_DUMP_BUF_LEN);
		return FALSE;
	}
...
wlmOTALoadTestFlowFile(char *flowfile)
{
	...
	args[1] = malloc(sizeof(char*) * (strlen(flowfile)+1));
	...
	memset(args[1], 0, sizeof(char*) * (strlen(flowfile)+1));
...
int
wlmConfigGet(const char *iovar, char *status_str, uint32 *val)
{
	wl_config_t *cfg;
	void *ptr = NULL;
	int i = 0;
