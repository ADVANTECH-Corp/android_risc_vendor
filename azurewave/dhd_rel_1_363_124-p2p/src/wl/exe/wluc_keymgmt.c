...
wl_addwep(void *wl, cmd_t *cmd, char **argv)
{
	...
	printf("%s: key.index %d key.len 0x%x key.algo 0x%x key.flags 0x%x\n",
			__FUNCTION__, key.index, key.len, key.algo, key.flags);

	printf("peer mac addr %s\n", wl_ether_etoa(&key.ea));
	prhex(NULL, (void *)key.data, key.len);
