...
static int writeline (const char *s)
{
    ...
    // Workaround for selective suspend issue
    s_copy = (char *)malloc(len + 1 + 1);
    if (s_copy == NULL)
    ...
    while (cur < len) {
        do {
            ...
            written = write (s_fd, s_copy + cur, len - cur);
    ...
    /* the \r  */
    // Removed because of the workaround
    /*
    do {
        written = write (s_fd, "\r" , 1);
    } while ((written < 0 && errno == EINTR) || (written == 0));
    RLOGD("AT> written: %d, errno %d\n", written, errno);
    */
 ...
 * at_response_free
 */
int at_send_command (const char *command, ATResponse **pp_outResponse, long long timeoutMsec)
{
    int err;

    if (timeoutMsec < 0)
      timeoutMsec = AT_TIMEOUT_NORMAL;
