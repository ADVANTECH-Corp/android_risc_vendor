...
static void skipNextColon(char **p_cur)
{
    if (*p_cur == NULL) return;
...
static char * nextTokBracket(char **p_cur)
{
    char *ret = NULL;
...
char * nextStrOccurrence(char **p_cur, char *str)
{
  ...
  if (*p_cur == NULL) {
        ...
        ret = strstr(*p_cur, str);
        if (ret != NULL) {
          ret += strlen(str);
          (*p_cur) = ret;
          //(*p_cur) = strstr(*p_cur, str);
        }
    }
...
/** returns next bracket token (works only without nested brackets) */
